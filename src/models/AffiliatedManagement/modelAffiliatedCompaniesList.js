import * as AffiliatedComponentListServices from '../../services/AffiliatedComponentList'
import { PAGE_SIZE } from '../../constants'
import { routerRedux } from 'dva/router';
import { warning, success, error } from '../../utils/noticeTips'

export default {
  namespace: 'affiliatedComponentListState',
  ...
  effects: {
    *fetch( { payload: { current } }, {call, put}){
      ...
      const data = yield call(AffiliatedComponentListServices.fetchAffiliatedComponentList, { current });
  ...
  subscriptions: {
    setup({dispatch, history}) {
      history.listen(({ pathname }) => {
        if (pathname === '/affiliatedCompanies') {
          dispatch({
            type: 'fetch',
            payload: {
              current: 1
