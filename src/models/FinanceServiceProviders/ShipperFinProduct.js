import * as FinanceProvidersInformationServices from '../../services/ShipperFinProduct'
...
export default {
  namespace: 'shipperFinProductInformation',
  ...
  effects: {
    *fetch( { payload: { current,car_type,province_id,city_id,service_company,userInfo } }, {call, put}){
      const data = yield call(FinanceProvidersInformationServices.fetchInformation, { current:current,car_type:car_type,province_id:province_id,city_id:city_id,service_company:service_company,userInfo:userInfo });
    ...
    *handleProductInformation( { payload: object }, {call, put}) {
      if(object.oprType == 'add' || object.oprType === 'edit'){
         const data = yield call(FinanceProvidersInformationServices.addProduct, object);
         if (data.code == 'SUCCESS') {
            if (object.oprType == 'add') {
              success('数据添加成功!');
              yield put({type: 'triggerModal'});
            } else if (object.oprType === 'edit') {
              success('修改数据!');
      ...
      }else if(object.oprType === 'fb'){
        const data = yield call(FinanceProvidersInformationServices.publishProduct, object);
        if (data.code == 'SUCCESS') {
            success('下架成功!');
            yield put({
              type: 'fetch',
        ...
        const data = yield call(FinanceProvidersInformationServices.delProduct, object);
        if (data.code == 'SUCCESS') {
            success('数据删除成功!');
            yield put({
              type: 'fetch',
  ...
  subscriptions: {
    setup({dispatch, history}) {
      history.listen(({ pathname }) => {
        if (pathname == '/shipperFinancialProducts') {
          dispatch({
            type: 'fetch',
            payload: {
              current: 1
