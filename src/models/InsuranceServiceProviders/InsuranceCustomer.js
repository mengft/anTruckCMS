import * as InsuranceProvidersInformationServices from '../../services/InsuranceCustomer'
...
export default {
  ...
  effects: {
    *fetch( { payload: { current,userInfo } }, {call, put}){
      const data = yield call(InsuranceProvidersInformationServices.fetchInsuranceInformation, { current:current,userInfo:userInfo });
  ...
  subscriptions: {
    setup({dispatch, history}) {
      history.listen(({ pathname }) => {
        if (pathname == '/insuranceCustomers') {
          dispatch({
            type: 'fetch',
            payload: {
              current: 1
