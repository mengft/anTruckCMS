import * as TrafficProvidersInformationServices from '../../services/TrafficProduct'
...
export default {
  namespace: 'trafficProductInformation',
  ...
  effects: {
    *fetch( { payload: { current,car_type,province_id,city_id,service_company,isTraffic,userInfo } }, {call, put}){
      if(isTraffic != 0 || isTraffic != "0" ){
        isTraffic=1;
      }
      const data = yield call(TrafficProvidersInformationServices.fetchTrafficInformation, { current:current,car_type:car_type,province_id:province_id,city_id:city_id,service_company:service_company,isTraffic:isTraffic,userInfo:userInfo });
    ...
    *handleTrafficProductInformation( { payload: object }, {call, put}) {
      if(object.type === 'fb'){
        const data = yield call(TrafficProvidersInformationServices.publishTrafficProduct, object);
        if (data.code == 'SUCCESS') {
            success('下架成功!');
            yield put({
              type: 'fetch',
        ...
        const data = yield call(TrafficProvidersInformationServices.delTrafficProduct, object);
        if (data.code == 'SUCCESS') {
            success('数据删除成功!');
            yield put({
              type: 'fetch',
