import * as DedicatedLineListServices from '../../services/DedicatedLineList'
import { PAGE_SIZE } from '../../constants'
import { routerRedux } from 'dva/router';
import { warning, success, error } from '../../utils/noticeTips'

export default {
  namespace: 'dedicatedLineState',
  ...
  effects: {
    *fetchDedicatedLine( { payload: { current } }, {call, put}){
      yield put({type: 'loading', payload: {}});
      const data = yield call(DedicatedLineListServices.fetchDedicatedLineList, { current });
  ...
  subscriptions: {
    setup({dispatch, history}) {
      history.listen(({ pathname }) => {
        if (pathname === '/dedicatedLineList') {
          dispatch({
            type: 'fetchDedicatedLine',
            payload: {
              current: 1
            }
          });
