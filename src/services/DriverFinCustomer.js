...
export function fetchInformation({current,userInfo}) {
  ...
  return request(`/api/admin/finance/customer/list?`+search, {headers: newHeaers()});
}
...
export function delCustomer({ id,company, contact_name, contact_tel, car_num, remark}) {
	return request(`/api/admin/finance/customer/delete?customerId=${id}`, {headers: newHeaers()});
