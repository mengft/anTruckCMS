...
export function fetchInformation({current,car_type,province_id,city_id,service_company,userInfo}) {
  ...
  return request(`/api/admin/finance_shipper/business/list?`+search, {headers: newHeaers()});
}
...
export function delProduct({ id}) {
  return request(`/api/admin/finance_shipper/business/delete?productId=${id}`, {headers: newHeaers()});
...
export function publishProduct({ id}) {
  return request(`/api/admin/finance_shipper/business/offsale?productId=${id}`, {headers: newHeaers()});
