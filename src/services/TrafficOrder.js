...
/*查询保险产品列表信息 */
export function fetchTrafficInformation({current,car_type,province_id,city_id,status,userInfo}) {
  current = current || 1;
  ...
  return request(`/api/admin/traffic/order/list?`+search, {headers: newHeaers()});
}
...
export function delTrafficOrder({ id}) {
	return request(`/api/admin/traffic/order/delete?orderId=${id}`, {headers: newHeaers()});
...
export function handleTrafficOrder({ id}) {
	return request(`/api/admin/traffic/order/handle?orderId=${id}`, {headers: newHeaers()});
